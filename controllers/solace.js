...
module.exports.deleteSolace = async (req, res, next)=>{
   console.log("entered");
    ...
    const solaceToDelete= await solace.findById(solace_id).populate('reviews');
    if(!solaceToDelete){
        console.log("no solace found");
    }
    await Review.deleteMany({_id:{$in:solaceToDelete.reviews}}); 
    // await solace.findByIdAndDelete(solace_id, async (err, doc)=>{
    //     await Review.deleteMany({_id : { $in : doc.reviews }});
    // }).populate('reviews');
    await solace.deleteOne({_id:solace_id});
    console.log("done");
    res.redirect(`/solaces`);   
